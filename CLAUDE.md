...
**HTTP only — deployed on `http://localhost:55000`, dev on `http://localhost:5080`.** There is no HTTPS (the admin UI is local-only). The binding comes from `Kestrel:Endpoints:Http:Url`: `appsettings.json` sets the **deployed/Production** port (55000), and `appsettings.Development.json` overrides it to 5080 for dev. The dev launch profile's `applicationUrl` (`launchSettings.json`, dev-only) also points at 5080 for the browser-launch. Note a `Kestrel:Endpoints` value takes precedence over `ASPNETCORE_URLS`/`applicationUrl`, so the per-environment port must be set via the config layer (not left to the launch profile to override).
...
- **Layout shell:** `MainLayout` wraps the top bar + `<main class="app-main">` in `.app-shell` (a flex column at `height: 100vh`), so `.app-main` fills the viewport below the title bar. Pages that want the old centred, max-width look opt into a `.page` wrapper (e.g. `Dashboard`); pages that need full height/bleed (e.g. `Settings`, with its left-docked `SettingsSidepanel`) use `.app-main` directly.
